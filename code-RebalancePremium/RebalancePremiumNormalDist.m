...
for i=1:nTrials
    stockReturns=randn(nYears,nStocks)*volatility+(expectedReturn+1);
    RebPortfolioReturns=mean(stockReturns,2);
    stocksCumVal=prod(stockReturns,1);
    RebPortfolioEndVal(i)=nStocks*1*prod(RebPortfolioReturns,1);
...
%subplot(1,2,1)
[nelements,centers] = hist(ValDiffChopped,100);
bar(centers,nelements);

[maxValue, maxLocation] = max(nelements);
xCenterLocation = centers(maxLocation);
line([xCenterLocation, xCenterLocation],get(gca,'YLim'),'Color',[0.5 0.5 0],'linestyle','-.')
...
Legend1=legend('Portfolio value difference','Zero line','mean of portfolio value difference','most likely portfolio value difference');
...
disp({'Most Likely $ Diff',xCenterLocation});
